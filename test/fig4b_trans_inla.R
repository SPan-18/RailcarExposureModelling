...
list.dat$T.obs = 16
list.dat$y = list.dat$y[1:16]
list.dat$G.ind = list.dat$G.ind[1:16]
...
x.time <- 1:T.end
C.dat <- c(list.dat$y, rep(NA, T.end - list.dat$T.obs))
...
jpeg("../fig/fig4b.jpg", width = 6.7, height = 4.47, units = "in", res = 300)
{
...
lines(exp(m.crw2$summary.fitted.values$'mean'), col = "mediumpurple4")
polygon(c(1:T.end, rev(1:T.end)),
        c(exp(m.crw2$summary.fitted.values$'0.975quant'), 
          rev(exp(m.crw2$summary.fitted.values$'0.025quant'))),
...
polygon(c(list.dat$T.obs:T.end, rev(list.dat$T.obs:T.end)),
        ...
        border = F)
points(1:40, c(rep(NA, list.dat$T.obs), 
               exp(obs1[(list.dat$T.obs+1):T.end])), pch = 4, col = "yellow")
