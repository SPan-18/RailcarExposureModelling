...
e.L = 0.6          # fraction of the source emissions immediately 
...
toy.vals = c(G, Q, V, T0, C0, Q.L, Q.R, e.L, e.LF, e.RF)
out1 = sapply(1:T.end, function(x) model111(x, params = toy.vals))
obs1 = sapply(out1, function(x) log(x) + rnorm(1, 0, sd.obs))

# plot.min = min(out1, exp(obs1))
# plot.max = max(out1, exp(obs1))
# plot(out1, type = "l", ylim = c(plot.min, plot.max))
# lines(exp(obs1), lty = 2, col = "red")

obsdf = data.frame("Conc" = obs1[1:T.obs],
                   "G.Ind" = rep(c(1,0), times = c(T0, T.obs - T0)),
                   "Time" = 1:T.obs)

# priorGQ = find.priors.101(df = exp(obsdf), V = V)
# G.prior = priorGQ[1,]
# Q.prior = priorGQ[2,]
G.prior = c(100, 3000)
Q.prior = c(2, 40)
